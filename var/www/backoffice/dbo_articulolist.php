...
class cdbo_articulo_list extends cdbo_articulo {
	...
	function __construct() {
		...
		$this->AddUrl = "dbo_articuloadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
	...
	function SetupListOptions() {
		...
		// "detail_trama_articulos2Dimagenes2Dadicionales"
		$item = &$this->ListOptions->Add("detail_trama_articulos2Dimagenes2Dadicionales");
		$item->CssStyle = "white-space: nowrap;";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'trama_articulos-imagenes-adicionales') && !$this->ShowMultipleDetails;
		$item->OnLeft = TRUE;
		...
		// Set up detail pages
		$pages = new cSubPages();
		$pages->Add("trama_articulos2Dimagenes2Dadicionales");
	...
	function RenderListOptions() {
		...
		// "detail_trama_articulos2Dimagenes2Dadicionales"
		$oListOpt = &$this->ListOptions->Items["detail_trama_articulos2Dimagenes2Dadicionales"];
		if ($Security->AllowList(CurrentProjectID() . 'trama_articulos-imagenes-adicionales')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("trama_articulos2Dimagenes2Dadicionales", "TblCaption");
			$body = "<a class=\"btn btn-default btn-sm ewRowLink ewDetail\" data-action=\"list\" href=\"" . ew_HtmlEncode("trama_articulos2Dimagenes2Dadicionaleslist.php?" . EW_TABLE_SHOW_MASTER . "=dbo_articulo&fk_CodInternoArti=" . urlencode(strval($this->CodInternoArti->CurrentValue)) . "") . "\">" . $body . "</a>";
			$links = "";
			if ($GLOBALS["trama_articulos2Dimagenes2Dadicionales_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'trama_articulos-imagenes-adicionales')) {
				$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=trama_articulos2Dimagenes2Dadicionales")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
				if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
				$DetailEditTblVar .= "trama_articulos2Dimagenes2Dadicionales";
			}
			if ($links <> "") {
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_trama_articulos2Dimagenes2Dadicionales");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=trama_articulos2Dimagenes2Dadicionales");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["trama_articulos2Dimagenes2Dadicionales"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["trama_articulos2Dimagenes2Dadicionales"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'trama_articulos-imagenes-adicionales') && $Security->CanAdd());
		if ($item->Visible) {
			if ($DetailTableLink <> "") $DetailTableLink .= ",";
			$DetailTableLink .= "trama_articulos2Dimagenes2Dadicionales";
		}
	...
	function ListOptions_Rendered() {
				$sql = "SELECT imagenes FROM `trama_articulos-imagenes-adicionales` WHERE idArticulo='".$this->CodInternoArti->DbValue."'";
				$rs = ew_LoadRecordset($sql);
				if ($rs->_numOfRows>0) {
					$rows = $rs->GetRows();
					if ($rows[0]["imagenes"]!=NULL) {
						$color='color:green';
					}else{
						$color='';
					}
					$this->ListOptions->Items["detail_trama_articulos2Dimagenes2Dadicionales"]->Body='<a class="btn btn-default btn-sm ewRowLink " data-action="list" data-toggle="tooltip" data-placement="bottom" title="Imágenes adicionales" href="trama_articulos2Dimagenes2Dadicionalesedit.php?idArticulo='.$this->CodInternoArti->DbValue.'&showmaster=dbo_articulo&fk_CodInternoArti='.$this->CodInternoArti->DbValue.'"><span style="'.$color.'" class="glyphicon glyphicon-picture ewIcon" aria-hidden="true"></span></a>';		
				}else{
					$this->ListOptions->Items["detail_trama_articulos2Dimagenes2Dadicionales"]->Body='<a class="btn btn-default btn-sm ewRowLink " data-action="list" data-toggle="tooltip" data-placement="bottom" title="Imágenes adicionales" href="trama_articulos2Dimagenes2Dadicionalesadd.php?showmaster=dbo_articulo&fk_CodInternoArti='.$this->CodInternoArti->DbValue.'"><span class="glyphicon glyphicon-picture ewIcon" aria-hidden="true"></span></a>';					
				}
				global $dbo_articulo_read_only;
...
{
	$(".ewListOptionHeader[data-name='button']").hide();
	$(".ewListOptionBody[data-name='button']").hide();
}
</script>
<?php } ?>
