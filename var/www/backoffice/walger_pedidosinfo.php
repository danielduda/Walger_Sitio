...
class cwalger_pedidos extends cTable {
	...
	function __construct() {
		...
		// medioEnvio
		$this->medioEnvio = new cField('walger_pedidos', 'walger_pedidos', 'x_medioEnvio', 'medioEnvio', '`medioEnvio`', '`medioEnvio`', 200, -1, FALSE, '`medioEnvio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->medioEnvio->Sortable = TRUE; // Allow sort
		$this->fields['medioEnvio'] = &$this->medioEnvio;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->medioEnvio->EditValue = $this->medioEnvio->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->medioEnvio->Exportable) $Doc->ExportCaption($this->medioEnvio);
				} else {
					if ($this->idPedido->Exportable) $Doc->ExportCaption($this->idPedido);
					if ($this->CodigoCli->Exportable) $Doc->ExportCaption($this->CodigoCli);
					if ($this->estado->Exportable) $Doc->ExportCaption($this->estado);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->medioEnvio->Exportable) $Doc->ExportField($this->medioEnvio);
					} else {
						if ($this->idPedido->Exportable) $Doc->ExportField($this->idPedido);
						if ($this->CodigoCli->Exportable) $Doc->ExportField($this->CodigoCli);
						if ($this->estado->Exportable) $Doc->ExportField($this->estado);
