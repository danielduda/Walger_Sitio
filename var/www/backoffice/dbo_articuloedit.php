...
$dbo_articulo_edit = NULL; // Initialize page object first

class cdbo_articulo_edit extends cdbo_articulo {
	...
	// Page object name
	var $PageObjName = 'dbo_articulo_edit';
	...
	function Page_Main() {
		...
		if ($this->CodInternoArti->CurrentValue == "") {
			$this->Page_Terminate("dbo_articulolist.php"); // Invalid key, return to list
		}
	...
	function RenderRow() {
		...
		$this->TasaIva->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->NombreFotoArti);
	...
	function ValidateForm() {
		...
		if (in_array("trama_articulos2Dimagenes2Dadicionales", $DetailTblVar) && $GLOBALS["trama_articulos2Dimagenes2Dadicionales"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->CodBarraArti->SetDbValueDef($rsnew, $this->CodBarraArti->CurrentValue, NULL, $this->CodBarraArti->ReadOnly);

			// idTipoArticulo
			$this->idTipoArticulo->SetDbValueDef($rsnew, $this->idTipoArticulo->CurrentValue, NULL, $this->idTipoArticulo->ReadOnly);

			// DescripcionArti
			$this->DescripcionArti->SetDbValueDef($rsnew, $this->DescripcionArti->CurrentValue, NULL, $this->DescripcionArti->ReadOnly);

			// detalle
			$this->detalle->SetDbValueDef($rsnew, $this->detalle->CurrentValue, NULL, $this->detalle->ReadOnly);

			// PrecioVta1_PreArti
			$this->PrecioVta1_PreArti->SetDbValueDef($rsnew, $this->PrecioVta1_PreArti->CurrentValue, NULL, $this->PrecioVta1_PreArti->ReadOnly);

			// Stock1_StkArti
			$this->Stock1_StkArti->SetDbValueDef($rsnew, $this->Stock1_StkArti->CurrentValue, NULL, $this->Stock1_StkArti->ReadOnly);

			// NombreFotoArti
			if ($this->NombreFotoArti->Visible && !$this->NombreFotoArti->ReadOnly && !$this->NombreFotoArti->Upload->KeepFile) {
				$this->NombreFotoArti->Upload->DbValue = $rsold['NombreFotoArti']; // Get original value
			...
			// DescrNivelInt4
			$this->DescrNivelInt4->SetDbValueDef($rsnew, $this->DescrNivelInt4->CurrentValue, NULL, $this->DescrNivelInt4->ReadOnly);

			// DescrNivelInt3
			$this->DescrNivelInt3->SetDbValueDef($rsnew, $this->DescrNivelInt3->CurrentValue, NULL, $this->DescrNivelInt3->ReadOnly);

			// DescrNivelInt2
			$this->DescrNivelInt2->SetDbValueDef($rsnew, $this->DescrNivelInt2->CurrentValue, NULL, $this->DescrNivelInt2->ReadOnly);

			// TasaIva
			$this->TasaIva->SetDbValueDef($rsnew, $this->TasaIva->CurrentValue, NULL, $this->TasaIva->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if (in_array("trama_articulos2Dimagenes2Dadicionales", $DetailTblVar) && $GLOBALS["trama_articulos2Dimagenes2Dadicionales"]->DetailEdit) {
						...
						$EditRow = $GLOBALS["trama_articulos2Dimagenes2Dadicionales_grid"]->GridUpdate();
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("trama_articulos2Dimagenes2Dadicionales", $DetailTblVar)) {
				...
				if ($GLOBALS["trama_articulos2Dimagenes2Dadicionales_grid"]->DetailEdit) {
					$GLOBALS["trama_articulos2Dimagenes2Dadicionales_grid"]->CurrentMode = "edit";
					$GLOBALS["trama_articulos2Dimagenes2Dadicionales_grid"]->CurrentAction = "gridedit";
...
// Create page object
if (!isset($dbo_articulo_edit)) $dbo_articulo_edit = new cdbo_articulo_edit();

// Page init
$dbo_articulo_edit->Page_Init();

// Page main
$dbo_articulo_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$dbo_articulo_edit->Page_Render();
?>
...
var CurrentForm = fdbo_articuloedit = new ew_Form("fdbo_articuloedit", "edit");

// Validate form
fdbo_articuloedit.Validate = function() {
...
fdbo_articuloedit.Form_CustomValidate = 
...
fdbo_articuloedit.ValidateRequired = true;
<?php } else { ?>
fdbo_articuloedit.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdbo_articuloedit.Lists["x_idTipoArticulo"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_denominacion","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"trama_tipos2Darticulos"};
...
<?php if (!$dbo_articulo_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $dbo_articulo_edit->ShowPageHeader(); ?>
<?php
$dbo_articulo_edit->ShowMessage();
?>
<form name="fdbo_articuloedit" id="fdbo_articuloedit" class="<?php echo $dbo_articulo_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($dbo_articulo_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $dbo_articulo_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $dbo_articulo->CodInternoArti->EditValue ?></p></span>
</span>
<input type="hidden" data-table="dbo_articulo" data-field="x_CodInternoArti" name="x_CodInternoArti" id="x_CodInternoArti" value="<?php echo ew_HtmlEncode($dbo_articulo->CodInternoArti->CurrentValue) ?>">
...
<script type="text/javascript">
ew_CreateEditor("fdbo_articuloedit", "x_detalle", 35, 4, <?php echo ($dbo_articulo->detalle->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if (@$_POST["fa_x_NombreFotoArti"] == "0") { ?>
<input type="hidden" name="fa_x_NombreFotoArti" id= "fa_x_NombreFotoArti" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_NombreFotoArti" id= "fa_x_NombreFotoArti" value="1">
<?php } ?>
...
<?php
	if (in_array("trama_articulos2Dimagenes2Dadicionales", explode(",", $dbo_articulo->getCurrentDetailTable())) && $trama_articulos2Dimagenes2Dadicionales->DetailEdit) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $dbo_articulo_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fdbo_articuloedit.Init();
</script>
<?php
$dbo_articulo_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$dbo_articulo_edit->Page_Terminate();
?>
