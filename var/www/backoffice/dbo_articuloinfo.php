...
class cdbo_articulo extends cTable {
	...
	function GetDetailUrl() {
		...
		// Detail url
		$sDetailUrl = "";
		if ($this->getCurrentDetailTable() == "trama_articulos2Dimagenes2Dadicionales") {
			$sDetailUrl = $GLOBALS["trama_articulos2Dimagenes2Dadicionales"]->GetListUrl() . "?" . EW_TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$sDetailUrl .= "&fk_CodInternoArti=" . urlencode($this->CodInternoArti->CurrentValue);
		}
		if ($sDetailUrl == "") {
			$sDetailUrl = "dbo_articulolist.php";
		}
		return $sDetailUrl;
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			...
			$url = $this->KeyUrl("dbo_articuloedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			...
			$url = $this->KeyUrl("dbo_articuloadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
		return $this->AddMasterUrl($url);
	}
