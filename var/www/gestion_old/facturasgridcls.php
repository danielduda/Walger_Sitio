...
$facturas_grid = NULL; // Initialize page object first

class cfacturas_grid extends cfacturas {
	...
	// Page object name
	var $PageObjName = 'facturas_grid';

	// Grid form hidden field names
	var $FormName = 'ffacturasgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["facturas"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->numFactura->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_numFactura") && $objForm->HasValue("o_numFactura") && $this->numFactura->CurrentValue <> $this->numFactura->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->numFactura->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->numFactura->setOldValue($objForm->GetValue("o_numFactura"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->numFactura->CurrentValue = strval($arKeys[0]); // numFactura
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
