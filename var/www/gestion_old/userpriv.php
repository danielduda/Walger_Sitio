...
class cuserpriv extends cniveles {
	...
	function Page_Main() {
		...
		$Breadcrumb->Add("list", "<span>" . $Language->TablePhrase("niveles", "TblCaption") . "</span>", "niveleslist.php", "niveles");
	...
	function EditRow() {
		...
		for ($i = 0; $i < $this->TableNameCount; $i++) {
			...
			if ($rs && !$rs->EOF) {
				...
				$Sql = "INSERT INTO " . EW_USER_LEVEL_PRIV_TABLE . " (" . EW_USER_LEVEL_PRIV_TABLE_NAME_FIELD . ", " . EW_USER_LEVEL_PRIV_USER_LEVEL_ID_FIELD . ", " . EW_USER_LEVEL_PRIV_PRIV_FIELD . ") VALUES ('" . ew_AdjustSql($this->TableList[$i][4] . $this->TableList[$i][0]) . "', " . $this->idNivel->CurrentValue . ", " . $this->Privileges[$i] . ")";
				$conn->Execute($Sql);
			}
			if ($rs)
				$rs->Close();
