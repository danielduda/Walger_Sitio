...
if (isset($_POST ["guardar"])){
	...
	$numrem=mysql_query($numrem);
	$numremf=mysql_fetch_assoc($numrem);
	$q = "INSERT INTO `remitos` (Fecha, Cliente, Proveedor, Transporte, NumeroDeBultos, OperadorTraslado,"." OperadorEntrego, OperadorVerifico, Observaciones, observacionesInternas, Importe, numeroRemito,tipodestinatario, estado)"." VALUES ('".$newDate."','".$_POST["x_Cliente"]."','".$_POST["x_Proveedor"]."','".$_POST["transporte"]."','".$_POST["bultos"]."','".$_POST["opTraslado"]."','".$_POST["opEntrego"]."','".$_POST["opVerifico"]."','".$_POST["observaciones"]."','".$_POST["observacionesint"]."','".$_POST["importe"]."','".$numremf["f"]."','".$_POST["tipodestinatario"]."','".$_POST["estado"]."')";
	$q = mysql_query($q);
	...
	for($i = 1; $i <= $MAXIMO_DETALLE; $i++){
		if ($_POST["x_Producto".$i] != ""){
			$q = "INSERT INTO `remitos_detalle` (remitoCabecera, cantidad, descripcion) " ."VALUES ('".$idCabecera."', '".$_POST["cantidadDetalle".$i]."','".$_POST["x_Producto".$i]."')";
			$q = mysql_query($q);
		}
	}
	for($i = 1; $i <= $MAXIMO_FACTURA; $i++){
		if ($_POST["factura".$i] != ""){
			$q = "INSERT INTO `facturas` (RemitoCabecera, numFactura) " ."VALUES ('".$idCabecera."', '".$_POST["factura".$i]."')";
			$q = mysql_query($q);
		}
	}
			...
			<div class="noseimprime noseimprimenada" id="estado" style="display: flex;">
				...
				<span>
					<select class="cli" name="estado" id="selectestado">
						<option value="0" selected>En trámite</option>
						<option value="1">Finalizado</option>
						<option value="2">Anulado</option>
						<option value="3">Editable</option>
					</select>
				</span>
			</div>
		...
		<table class="ltable">
			...
			for ($i = 1; $i <= $MAXIMO_DETALLE; $i ++){
				...
				<tr>
					...
					<td>
						<select class="descripcion noseimprime" data-field="x_Producto" id="x_Producto" name="x_Producto<?PHP echo($i); ?>">
							<option value="" selected="selected">Seleccione</option>
							<?php
