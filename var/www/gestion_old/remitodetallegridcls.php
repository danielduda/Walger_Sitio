...
$remitodetalle_grid = NULL; // Initialize page object first

class cremitodetalle_grid extends cremitodetalle {
	...
	// Page object name
	var $PageObjName = 'remitodetalle_grid';

	// Grid form hidden field names
	var $FormName = 'fremitodetallegrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["remitodetalle"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->Id_RemitoDetalle->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
					$sKey .= $this->CodigoCli->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_remitoCabecera") && $objForm->HasValue("o_remitoCabecera") && $this->remitoCabecera->CurrentValue <> $this->remitoCabecera->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_descripcion") && $objForm->HasValue("o_descripcion") && $this->descripcion->CurrentValue <> $this->descripcion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_CodigoCli") && $objForm->HasValue("o_CodigoCli") && $this->CodigoCli->CurrentValue <> $this->CodigoCli->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_RazonSocialCli") && $objForm->HasValue("o_RazonSocialCli") && $this->RazonSocialCli->CurrentValue <> $this->RazonSocialCli->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_Direccion") && $objForm->HasValue("o_Direccion") && $this->Direccion->CurrentValue <> $this->Direccion->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_LocalidadCli") && $objForm->HasValue("o_LocalidadCli") && $this->LocalidadCli->CurrentValue <> $this->LocalidadCli->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->Id_RemitoDetalle->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->CodigoCli->CurrentValue . "\">";
		}
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('Id_RemitoDetalle');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('CodigoCli');
	}
	...
	function LoadFormValues() {
		...
		$this->remitoCabecera->setOldValue($objForm->GetValue("o_remitoCabecera"));
		...
		$this->CodigoCli->setOldValue($objForm->GetValue("o_CodigoCli"));
		if (!$this->RazonSocialCli->FldIsDetailKey) {
			$this->RazonSocialCli->setFormValue($objForm->GetValue("x_RazonSocialCli"));
		}
		$this->RazonSocialCli->setOldValue($objForm->GetValue("o_RazonSocialCli"));
		...
		$this->LocalidadCli->setOldValue($objForm->GetValue("o_LocalidadCli"));
		if (!$this->Id_RemitoDetalle->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->Id_RemitoDetalle->CurrentValue = strval($arKeys[0]); // Id_RemitoDetalle
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->CodigoCli->CurrentValue = strval($arKeys[1]); // CodigoCli
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "remitos") {
				$this->remitoCabecera->CurrentValue = $this->remitoCabecera->getSessionValue();
			}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "remitos") {
			$this->remitoCabecera->Visible = FALSE;
			if ($GLOBALS["remitos"]->EventCancelled) $this->EventCancelled = TRUE;
		}
