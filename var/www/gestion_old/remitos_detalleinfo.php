...
$remitos_detalle = NULL;

//
// Table class for remitos_detalle
//
class cremitos_detalle extends cTable {
	...
	function __construct() {
		...
		$this->TableVar = 'remitos_detalle';
		$this->TableName = 'remitos_detalle';
		...
		// Id_RemitoDetalle
		$this->Id_RemitoDetalle = new cField('remitos_detalle', 'remitos_detalle', 'x_Id_RemitoDetalle', 'Id_RemitoDetalle', '`Id_RemitoDetalle`', '`Id_RemitoDetalle`', 19, -1, FALSE, '`Id_RemitoDetalle`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		...
		// remitoCabecera
		$this->remitoCabecera = new cField('remitos_detalle', 'remitos_detalle', 'x_remitoCabecera', 'remitoCabecera', '`remitoCabecera`', '`remitoCabecera`', 3, -1, FALSE, '`remitoCabecera`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->remitoCabecera->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['remitoCabecera'] = &$this->remitoCabecera;

		// cantidad
		$this->cantidad = new cField('remitos_detalle', 'remitos_detalle', 'x_cantidad', 'cantidad', '`cantidad`', '`cantidad`', 200, -1, FALSE, '`cantidad`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['cantidad'] = &$this->cantidad;

		// descripcion
		$this->descripcion = new cField('remitos_detalle', 'remitos_detalle', 'x_descripcion', 'descripcion', '`descripcion`', '`descripcion`', 3, -1, FALSE, '`descripcion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetMasterFilter() {
		...
		if ($this->getCurrentMasterTable() == "remitos") {
			if ($this->remitoCabecera->getSessionValue() <> "")
				$sMasterFilter .= "`Id_Remito`=" . ew_QuotedValue($this->remitoCabecera->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "remitos") {
			if ($this->remitoCabecera->getSessionValue() <> "")
				$sDetailFilter .= "`remitoCabecera`=" . ew_QuotedValue($this->remitoCabecera->getSessionValue(), EW_DATATYPE_NUMBER);
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_remitos() {
		return "`Id_Remito`=@Id_Remito@";
	}

	// Detail filter
	function SqlDetailFilter_remitos() {
		return "`remitoCabecera`=@remitoCabecera@";
	}

	// Table level SQL
	function SqlFrom() { // From
		return "`remitos_detalle`";
	}
	...
	// Update Table
	var $UpdateTable = "`remitos_detalle`";
	...
	function SqlKeyFilter() {
		return "`Id_RemitoDetalle` = @Id_RemitoDetalle@";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "remitos_detallelist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("remitos_detalleview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("remitos_detalleview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl() {
		return "remitos_detalleadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("remitos_detalleedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("remitos_detalleadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("remitos_detalledelete.php", $this->UrlParm());
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["Id_RemitoDetalle"]; // Id_RemitoDetalle
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->Id_RemitoDetalle->CurrentValue = $key;
