...
$niveles_add = NULL; // Initialize page object first

class cniveles_add extends cniveles {
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->idNivel->PlaceHolder = ew_HtmlEncode(ew_RemoveHtml($this->idNivel->FldCaption()));
	...
	function AddRow($rsold = NULL) {
		...
		} elseif (trim($this->denominacion->CurrentValue) == "") {
			$this->setFailureMessage($Language->Phrase("MissingUserLevelName"));
		} elseif (!is_numeric($this->idNivel->CurrentValue)) {
			$this->setFailureMessage($Language->Phrase("UserLevelIDInteger"));
		} elseif (intval($this->idNivel->CurrentValue) < -1) {
			$this->setFailureMessage($Language->Phrase("UserLevelIDIncorrect"));
		} elseif (intval($this->idNivel->CurrentValue) == 0 && strtolower(trim($this->denominacion->CurrentValue)) <> "default") {
			$this->setFailureMessage($Language->Phrase("UserLevelDefaultName"));
		} elseif (intval($this->idNivel->CurrentValue) == -1 && strtolower(trim($this->denominacion->CurrentValue)) <> "administrator") {
			$this->setFailureMessage($Language->Phrase("UserLevelAdministratorName"));
		} elseif (intval($this->idNivel->CurrentValue) > 0 && (strtolower(trim($this->denominacion->CurrentValue)) == "administrator" || strtolower(trim($this->denominacion->CurrentValue)) == "default")) {
		...
		if ($bInsertRow && $this->ValidateKey && $this->idNivel->CurrentValue == "" && $this->idNivel->getSessionValue() == "") {
		...
		if ($this->Priv > 0) {
			...
			for ($i = 0; $i < $TableNameCount; $i++) {
				$sSql = "INSERT INTO " . EW_USER_LEVEL_PRIV_TABLE . " (" .
					...
					"', " . $this->idNivel->CurrentValue . ", " . $this->Priv . ")";
				$conn->Execute($sSql);
			}
		}
		return $AddRow;
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", "<span id=\"ewPageCaption\">" . $PageCaption . "</span>", "niveleslist.php", $this->TableVar);
...
// Create page object
if (!isset($niveles_add)) $niveles_add = new cniveles_add();
...
// Form object
var fnivelesadd = new ew_Form("fnivelesadd");

// Validate form
fnivelesadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($niveles->idNivel->FldCaption()) ?>");
			elm = this.GetElements("x" + infix + "_idNivel");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($niveles->idNivel->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_denominacion");
			if (elm && !ew_HasValue(elm))
				return this.OnError(elm, ewLanguage.Phrase("EnterRequiredField") + " - <?php echo ew_JsEncode2($niveles->denominacion->FldCaption()) ?>");
			var elId = fobj.elements["x" + infix + "_idNivel"];
			var elName = fobj.elements["x" + infix + "_denominacion"];
			if (elId && elName) {
...
fnivelesadd.Form_CustomValidate = 
...
fnivelesadd.ValidateRequired = true;
<?php } else { ?>
fnivelesadd.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="fnivelesadd" id="fnivelesadd" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="niveles">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td>
<table id="tbl_nivelesadd" class="table table-bordered table-striped">
<?php if ($niveles->idNivel->Visible) { // idNivel ?>
	<tr id="r_idNivel">
		<td><span id="elh_niveles_idNivel"><?php echo $niveles->idNivel->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $niveles->idNivel->CellAttributes() ?>>
<span id="el_niveles_idNivel" class="control-group">
<input type="text" data-field="x_idNivel" name="x_idNivel" id="x_idNivel" size="30" placeholder="<?php echo $niveles->idNivel->PlaceHolder ?>" value="<?php echo $niveles->idNivel->EditValue ?>"<?php echo $niveles->idNivel->EditAttributes() ?>>
</span>
<?php echo $niveles->idNivel->CustomMsg ?></td>
	</tr>
<?php } ?>
<?php if ($niveles->denominacion->Visible) { // denominacion ?>
	<tr id="r_denominacion">
		<td><span id="elh_niveles_denominacion"><?php echo $niveles->denominacion->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></span></td>
		<td<?php echo $niveles->denominacion->CellAttributes() ?>>
<span id="el_niveles_denominacion" class="control-group">
<input type="text" data-field="x_denominacion" name="x_denominacion" id="x_denominacion" size="30" maxlength="255" placeholder="<?php echo $niveles->denominacion->PlaceHolder ?>" value="<?php echo $niveles->denominacion->EditValue ?>"<?php echo $niveles->denominacion->EditAttributes() ?>>
</span>
<?php echo $niveles->denominacion->CustomMsg ?></td>
	</tr>
<?php } ?>
