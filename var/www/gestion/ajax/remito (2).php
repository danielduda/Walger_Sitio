...
// elimino errores de deprecaci�n (OJO)
error_reporting (E_STRICT);

// conecto a la base de datos
$link_aux = mysqli_connect(
...
if (isset($_POST ["guardar"]))
{
	...
	$q =  "INSERT INTO remitos (Fecha, Cliente, Proveedor, Transporte, NumeroDeBultos, OperadorTraslado,".
		" OperadorEntrego, OperadorVerifico, Observaciones, observacionesInternas, Importe)".
" VALUES ('".$_POST["x_Fecha"]."','".$_POST["x_Cliente"]."','".$_POST["x_Proveedor"]."','".$_POST["transporte"]."','".$_POST["bultos"]."','".$_POST["opTraslado"]."','".$_POST["opEntrego"]."','".$_POST["opVerifico"]."','".$_POST["observaciones"]."','".$_POST["observacionesint"]."','".$_POST["importe"]."')";
...
{
	$f2[$i] = mysqli_fetch_array($q2, MYSQLI_ASSOC);
}
			...
			<tr>
				...
				<td class="encabezado2">
					...
					<div style="display: flex;">
						<span><h3>N�</h3></span>
						<span class="input-append" style="margin-left: 58px;"><input style="height: 35px;font-size: 20px" type="text" value=""/></span>
					</div>
					<div style="display: flex;">
						<span>
							<h3>FECHA</h3>
						...
						<span id="el_remitos_Fecha" class="control-group" style="margin-left: 10px;margin-bottom:0px">
							<span class="input-append" style="margin-bottom:0px">
								<span class="input-append" style="height: 25px;">
									<input type="text" data-field="x_Fecha" name="x_Fecha" id="x_Fecha" placeholder="Fecha"  style="height: 35px;font-size: 20px;" required>
							...
							<script type="text/javascript">
							ew_CreateCalendar("remitoadd", "x_Fecha", "%Y-%m-%d");
							</script>
						</span>
					</div>
...
function mostrarDatos ($resultados) {
	if ($resultados !=NULL) {
		?>	<option value=<?php echo $resultados['CodigoCli']; ?>><?php echo $resultados['RazonSocialCli']; ?></option>
		<?php
	}
}
...
<table class="ltable">
	...
	for ($i = 1; $i <= $MAXIMO_DETALLE; $i ++)
	{
		...
		<tr>
			<td>
				<input class="cantidad" type="text" value="" name="cantidadDetalle<?PHP echo($i); ?>" />
			</td>
			<td>
...
<td>
	...
	<span><select class="cli" data-field="x_Proveedor" id="x_Proveedor" name="opEntrego">
		<option value="" selected="selected">Seleccione</option>
		<?php
...
<td>
	...
	<span><select class="cli" data-field="x_Proveedor" id="x_Proveedor" name="transporte" >
		<option value="" selected="selected">Seleccione</option>
		<?php
...
<td>
	...
	<span><select class="cli" data-field="x_Proveedor" id="x_Proveedor" name="opVerifico">
		<option value="" selected="selected">Seleccione</option>
		<?php
...
<tr>
	<td>
		...
		<span><input id="bultos" class="cli" type="text" value="" name="bultos" /><br /></span>
	</td>
	<td>
		<span class="clie">Op Traslad�</span>
		<span><select class="cli" data-field="x_Proveedor" id="x_Proveedor" name="opTraslado">
		<option value="" selected="selected">Seleccione</option>
		<?php
...
<tr>
	<td>
		...
		<span><input class="cli" type="text" value="" name="importe" required/><br /></span>
	</td>
</tr>
...
<table class="ltable">
	...
	for ($i = 1; $i <= $MAXIMO_FACTURA; $i ++)
	{
		...
		<tr>
			<td>
				<input type="text" value="" name="factura<?PHP echo($i); ?>" />
			</td>
		</tr>
		<?PHP 
	}
