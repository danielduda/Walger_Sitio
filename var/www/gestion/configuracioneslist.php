...
<?php include_once "configuracionesinfo.php" ?>
...
class cconfiguraciones_list extends cconfiguraciones {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["configuraciones"])) {
		...
		$this->AddUrl = "configuracionesadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "configuracionesdelete.php";
		$this->MultiUpdateUrl = "configuracionesupdate.php";
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_configuraciones\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_configuraciones',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fconfiguracioneslist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
	...
	echo '<div id="autoincrement" style="padding: 20px;">';
	echo '<h3>Numeración de remitos</h3>'; 
		echo '<form action="configuracioneslist.php" method="post">';
					 if (isset($_POST ["actualizar"]))
					{
					$ultimoremito="SELECT max(numeroRemito) as f FROM `remitos`";
					$ultimoremito= mysqli_query($GLOBALS['CURRENT_CONNECTION_MYSQL'], $ultimoremito);
					$ultimoremitof=mysqli_fetch_assoc($ultimoremito);
					$proximo=$_POST["proximoid"];
					if ($proximo>$ultimoremitof["f"]) {
					$qu="UPDATE `numerador` SET ultimoNumero='".$proximo."' where nombre='remitos'";
					$qu=mysqli_query($GLOBALS['CURRENT_CONNECTION_MYSQL'], $qu);
					}else{
						echo "El número ingresado debe ser mayor al último remito </br>";
					}
					 }                                              
		$q = "select ultimoNumero as f from numerador where nombre='remitos'";
		$q=mysqli_query($GLOBALS['CURRENT_CONNECTION_MYSQL'], $q);
		$f=mysqli_fetch_assoc($q);
		echo '<div class="input-append">';
		echo '<input id="proxid" class="input-mini" name="proximoid" type="text" value="'.$f["f"].'">';
		echo '<input class="btn btn-primary ewButton" name="actualizar" type="submit" value="Actualzar numeración" onClick="window.location.reload()">';
		$q="select ultimoNumero as f from numerador";
		$q=mysqli_query($GLOBALS['CURRENT_CONNECTION_MYSQL'], $q);
		$proxf=mysqli_fetch_assoc($q);
		echo '<h4>Próximo número de remito 0001-'.str_pad($proxf["f"], 8, "0", STR_PAD_LEFT).'</h4>';
		echo '</div>';
		echo '</form>';
		echo '</div>';  
	}   
...
<?php } ?>
<?php if ($configuraciones_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $configuraciones_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		...
		if ($configuraciones_list->Recordset = $configuraciones_list->LoadRecordset())
...
<!--first page button-->
	...
	<a class="btn btn-small" href="<?php echo $configuraciones_list->PageUrl() ?>start=<?php echo $configuraciones_list->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
...
<!--previous page button-->
	...
	<a class="btn btn-small" href="<?php echo $configuraciones_list->PageUrl() ?>start=<?php echo $configuraciones_list->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small" disabled="disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $configuraciones_list->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($configuraciones_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $configuraciones_list->PageUrl() ?>start=<?php echo $configuraciones_list->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small" disabled="disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($configuraciones_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $configuraciones_list->PageUrl() ?>start=<?php echo $configuraciones_list->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small" disabled="disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $configuraciones_list->Pager->PageCount ?>
</td>
<td>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $configuraciones_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $configuraciones_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $configuraciones_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($configuraciones_list->SearchWhere == "0=101") { ?>
...
<?php } ?>
<form name="fconfiguracioneslist" id="fconfiguracioneslist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="configuraciones">
<div id="gmp_configuraciones" class="ewGridMiddlePanel">
<?php if ($configuraciones_list->TotalRecs > 0) { ?>
<table id="tbl_configuracioneslist" class="ewTable ewTableSeparate">
...
?>
<?php if ($configuraciones->_1->Visible) { // 1 ?>
	<?php if ($configuraciones->SortUrl($configuraciones->_1) == "") { ?>
		<td><div id="elh_configuraciones__1" class="configuraciones__1"><div class="ewTableHeaderCaption"><?php echo $configuraciones->_1->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $configuraciones->SortUrl($configuraciones->_1) ?>',1);"><div id="elh_configuraciones__1" class="configuraciones__1">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $configuraciones->_1->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($configuraciones->_1->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($configuraciones->_1->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php
...
?>
	<?php if ($configuraciones->_1->Visible) { // 1 ?>
		<td<?php echo $configuraciones->_1->CellAttributes() ?>>
<span<?php echo $configuraciones->_1->ViewAttributes() ?>>
<?php echo $configuraciones->_1->ListViewValue() ?></span>
...
$(document).ready(function(){
	$("td.ewGridContent").css("display","none");
});
</script>
<?php } ?>
