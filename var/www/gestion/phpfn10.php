...
function CurrentProjectID() {
	...
	return "{B81C6C2E-1100-4548-836E-685E96F6B551}";
}
...
class cAdvancedSecurity {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setSessionParentUserID($rs->fields('idUsuario')); // Load parent User ID
						if (is_null($rs->fields('idNivel'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('idNivel'))); // Load User Level
						}
						$this->SetUpUserLevel();
	...
	function SetUpUserLevelEx() {
		...
		if (count($this->UserLevelPriv) == 0 && $this->IsAdmin() && $Page != NULL && @$_SESSION[EW_SESSION_USER_LEVEL_MSG] == "") {
			...
			$Page->Page_Terminate("niveleslist.php");
		}
		return TRUE;
	}
		...
		if (strval($this->CurrentUserID) == "") {
			...
			if (EW_USER_ID_IS_HIERARCHICAL) {
				...
				while ($sUserIDList <> $sCurUserIDList) {
					$sFilter = '`idUsuario` IN (' . $sCurUserIDList . ')';
					$sSql = $usuarios->GetSQL($sFilter, '');
					if ($RsUser = $conn->Execute($sSql)) {
						while (!$RsUser->EOF) {
							$this->AddUserID($RsUser->fields('idUsuario'));
