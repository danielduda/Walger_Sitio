...
define('EW_USE_MYSQLI', TRUE, TRUE);
	...
	function _connect($host = "", $username = "", $password = "", $database = "", $persistent, $forcenew)
	{
		if (EW_USE_MYSQLI) {
		  ...
		  $this->connectionId = @mysqli_init();
			...
			{
				$this->connectionId = @mysqli_connect( "p:".$this->host, $this->username, $this->password, $this->clientFlags );
			}
			else
			{
				$this->connectionId = @mysqli_connect( $this->host, $this->username, $this->password, $this->forcenewconnection, $this->clientFlags );
			}
		}
		...
		$GLOBALS['CURRENT_CONNECTION_MYSQL'] = $this->connectionId;
		return true;
	} 
	...
	function SelectDB($dbname)
	{
		...
		else
		{
			if (EW_USE_MYSQLI) {
				...
				$result = @mysqli_select_db( $this->database, $this->connectionId );
			}
			if($result === false)
			{
				if($this->createdatabase == true)
				{
					if (EW_USE_MYSQLI) {
						...
						$result = @mysqli_query( "CREATE DATABASE IF NOT EXISTS " . $this->database, $this->connectionId );
					}
	...
	function qstr($string, $magic_quotes=false)
	{
		if (!$magic_quotes) {
			if (EW_USE_MYSQLI) {
				...
				return "'" . mysqli_real_escape_string($string, $this->connectionId) . "'";
			}
		...
		if ($inputarr && is_array($inputarr)) {
			...
			foreach($inputarr as $arr) {
				...
				if (EW_USE_MYSQLI) {
					...
					$resultId = @mysqli_query( $this->sql, $this->connectionId );
				}
