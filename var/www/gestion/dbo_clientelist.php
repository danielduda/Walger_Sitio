...
class cdbo_cliente_list extends cdbo_cliente {
	...
	function __construct() {
		...
		$this->AddUrl = "dbo_clienteadd.php";
	...
	function BasicSearchSQL($Keyword) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->CuitCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->IngBrutosCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->emailCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->RazonSocialFlete, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->Direccion, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->BarrioCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->LocalidadCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->DescrProvincia, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->CodigoPostalCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->DescrPais, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->Telefono, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->FaxCli, $Keyword);
		$this->BuildBasicSearchSQL($sWhere, $this->PaginaWebCli, $Keyword);
		return $sWhere;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->CuitCli); // CuitCli
			$this->UpdateSort($this->IngBrutosCli); // IngBrutosCli
			$this->UpdateSort($this->Regis_IvaC); // Regis_IvaC
			$this->UpdateSort($this->Regis_ListaPrec); // Regis_ListaPrec
			$this->UpdateSort($this->emailCli); // emailCli
			$this->UpdateSort($this->RazonSocialFlete); // RazonSocialFlete
			$this->UpdateSort($this->Direccion); // Direccion
			$this->UpdateSort($this->BarrioCli); // BarrioCli
			$this->UpdateSort($this->LocalidadCli); // LocalidadCli
			$this->UpdateSort($this->DescrProvincia); // DescrProvincia
			$this->UpdateSort($this->CodigoPostalCli); // CodigoPostalCli
			$this->UpdateSort($this->DescrPais); // DescrPais
			$this->UpdateSort($this->Telefono); // Telefono
			$this->UpdateSort($this->FaxCli); // FaxCli
			$this->UpdateSort($this->PaginaWebCli); // PaginaWebCli
	...
	function LoadSortOrder() {
		...
		if ($sOrderBy == "") {
			if ($this->SqlOrderBy() <> "") {
				...
				$this->RazonSocialCli->setSort("ASC");
			}
		}
	}
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<label class=\"checkbox\"><input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->CodigoCli->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'></label>";
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fdbo_clientelist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_dbo_cliente\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_dbo_cliente',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fdbo_clientelist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
...
// Page object
var dbo_cliente_list = new ew_Page("dbo_cliente_list");
dbo_cliente_list.PageID = "list"; // Page ID
var EW_PAGE_ID = dbo_cliente_list.PageID; // For backward compatibility

// Form object
var fdbo_clientelist = new ew_Form("fdbo_clientelist");
...
var fdbo_clientelistsrch = new ew_Form("fdbo_clientelistsrch");
</script>
<script type="text/javascript">
...
<?php } ?>
<?php if ($dbo_cliente_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $dbo_cliente_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php
	$bSelectLimit = EW_SELECT_LIMIT;
	if ($bSelectLimit) {
		...
		if ($dbo_cliente_list->Recordset = $dbo_cliente_list->LoadRecordset())
...
<form name="fdbo_clientelistsrch" id="fdbo_clientelistsrch" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewSearchTable"><tr><td>
<div class="accordion" id="fdbo_clientelistsrch_SearchGroup">
	<div class="accordion-group">
		<div class="accordion-heading">
<a class="accordion-toggle" data-toggle="collapse" data-parent="#fdbo_clientelistsrch_SearchGroup" href="#fdbo_clientelistsrch_SearchBody"><?php echo $Language->Phrase("Search") ?></a>
		</div>
		<div id="fdbo_clientelistsrch_SearchBody" class="accordion-body collapse in">
			<div class="accordion-inner">
<div id="fdbo_clientelistsrch_SearchPanel">
...
<div id="xsr_1" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="input-large" value="<?php echo ew_HtmlEncode($dbo_cliente_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo $Language->Phrase("Search") ?>">
	<button class="btn btn-primary ewButton" name="btnsubmit" id="btnsubmit" type="submit"><?php echo $Language->Phrase("QuickSearchBtn") ?></button>
	</div>
	</div>
	<div class="btn-group ewButtonGroup">
	<a class="btn ewShowAll" href="<?php echo $dbo_cliente_list->PageUrl() ?>cmd=reset"><?php echo $Language->Phrase("ShowAll") ?></a>
</div>
<div id="xsr_2" class="ewRow">
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="="<?php if ($dbo_cliente_list->BasicSearch->getType() == "=") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("ExactPhrase") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="AND"<?php if ($dbo_cliente_list->BasicSearch->getType() == "AND") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AllWord") ?></label>
	<label class="inline radio ewRadio" style="white-space: nowrap;"><input type="radio" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="OR"<?php if ($dbo_cliente_list->BasicSearch->getType() == "OR") { ?> checked="checked"<?php } ?>><?php echo $Language->Phrase("AnyWord") ?></label>
</div>
</div>
</div>
			</div>
...
<?php if ($dbo_cliente_list->Pager->RecordCount > 0) { ?>
<table cellspacing="0" class="ewStdTable"><tbody><tr><td>
	<?php echo $Language->Phrase("Page") ?>&nbsp;
<div class="input-prepend input-append">
<!--first page button-->
	<?php if ($dbo_cliente_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $dbo_cliente_list->PageUrl() ?>start=<?php echo $dbo_cliente_list->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
...
<!--previous page button-->
	...
	<a class="btn btn-small" href="<?php echo $dbo_cliente_list->PageUrl() ?>start=<?php echo $dbo_cliente_list->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small" disabled="disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $dbo_cliente_list->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($dbo_cliente_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $dbo_cliente_list->PageUrl() ?>start=<?php echo $dbo_cliente_list->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small" disabled="disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($dbo_cliente_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $dbo_cliente_list->PageUrl() ?>start=<?php echo $dbo_cliente_list->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small" disabled="disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $dbo_cliente_list->Pager->PageCount ?>
</td>
<td>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $dbo_cliente_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $dbo_cliente_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $dbo_cliente_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($Security->CanList()) { ?>
	<?php if ($dbo_cliente_list->SearchWhere == "0=101") { ?>
...
<?php } ?>
<form name="fdbo_clientelist" id="fdbo_clientelist" class="ewForm form-horizontal" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="dbo_cliente">
<div id="gmp_dbo_cliente" class="ewGridMiddlePanel">
<?php if ($dbo_cliente_list->TotalRecs > 0) { ?>
<table id="tbl_dbo_clientelist" class="ewTable ewTableSeparate">
...
<?php if ($dbo_cliente->CodigoCli->Visible) { // CodigoCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->CodigoCli) == "") { ?>
		<td><div id="elh_dbo_cliente_CodigoCli" class="dbo_cliente_CodigoCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->CodigoCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->CodigoCli) ?>',1);"><div id="elh_dbo_cliente_CodigoCli" class="dbo_cliente_CodigoCli">
...
<?php if ($dbo_cliente->RazonSocialCli->Visible) { // RazonSocialCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->RazonSocialCli) == "") { ?>
		<td><div id="elh_dbo_cliente_RazonSocialCli" class="dbo_cliente_RazonSocialCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->RazonSocialCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->RazonSocialCli) ?>',1);"><div id="elh_dbo_cliente_RazonSocialCli" class="dbo_cliente_RazonSocialCli">
...
<?php if ($dbo_cliente->CuitCli->Visible) { // CuitCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->CuitCli) == "") { ?>
		<td><div id="elh_dbo_cliente_CuitCli" class="dbo_cliente_CuitCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->CuitCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->CuitCli) ?>',1);"><div id="elh_dbo_cliente_CuitCli" class="dbo_cliente_CuitCli">
...
<?php if ($dbo_cliente->IngBrutosCli->Visible) { // IngBrutosCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->IngBrutosCli) == "") { ?>
		<td><div id="elh_dbo_cliente_IngBrutosCli" class="dbo_cliente_IngBrutosCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->IngBrutosCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->IngBrutosCli) ?>',1);"><div id="elh_dbo_cliente_IngBrutosCli" class="dbo_cliente_IngBrutosCli">
...
<?php if ($dbo_cliente->Regis_IvaC->Visible) { // Regis_IvaC ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->Regis_IvaC) == "") { ?>
		<td><div id="elh_dbo_cliente_Regis_IvaC" class="dbo_cliente_Regis_IvaC"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->Regis_IvaC->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->Regis_IvaC) ?>',1);"><div id="elh_dbo_cliente_Regis_IvaC" class="dbo_cliente_Regis_IvaC">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $dbo_cliente->Regis_IvaC->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($dbo_cliente->Regis_IvaC->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($dbo_cliente->Regis_IvaC->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></td>
	<?php } ?>
<?php } ?>		
<?php if ($dbo_cliente->Regis_ListaPrec->Visible) { // Regis_ListaPrec ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->Regis_ListaPrec) == "") { ?>
		<td><div id="elh_dbo_cliente_Regis_ListaPrec" class="dbo_cliente_Regis_ListaPrec"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->Regis_ListaPrec->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->Regis_ListaPrec) ?>',1);"><div id="elh_dbo_cliente_Regis_ListaPrec" class="dbo_cliente_Regis_ListaPrec">
...
<?php if ($dbo_cliente->emailCli->Visible) { // emailCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->emailCli) == "") { ?>
		<td><div id="elh_dbo_cliente_emailCli" class="dbo_cliente_emailCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->emailCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->emailCli) ?>',1);"><div id="elh_dbo_cliente_emailCli" class="dbo_cliente_emailCli">
...
<?php if ($dbo_cliente->RazonSocialFlete->Visible) { // RazonSocialFlete ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->RazonSocialFlete) == "") { ?>
		<td><div id="elh_dbo_cliente_RazonSocialFlete" class="dbo_cliente_RazonSocialFlete"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->RazonSocialFlete->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->RazonSocialFlete) ?>',1);"><div id="elh_dbo_cliente_RazonSocialFlete" class="dbo_cliente_RazonSocialFlete">
...
<?php if ($dbo_cliente->Direccion->Visible) { // Direccion ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->Direccion) == "") { ?>
		<td><div id="elh_dbo_cliente_Direccion" class="dbo_cliente_Direccion"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->Direccion->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->Direccion) ?>',1);"><div id="elh_dbo_cliente_Direccion" class="dbo_cliente_Direccion">
...
<?php if ($dbo_cliente->BarrioCli->Visible) { // BarrioCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->BarrioCli) == "") { ?>
		<td><div id="elh_dbo_cliente_BarrioCli" class="dbo_cliente_BarrioCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->BarrioCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->BarrioCli) ?>',1);"><div id="elh_dbo_cliente_BarrioCli" class="dbo_cliente_BarrioCli">
...
<?php if ($dbo_cliente->LocalidadCli->Visible) { // LocalidadCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->LocalidadCli) == "") { ?>
		<td><div id="elh_dbo_cliente_LocalidadCli" class="dbo_cliente_LocalidadCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->LocalidadCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->LocalidadCli) ?>',1);"><div id="elh_dbo_cliente_LocalidadCli" class="dbo_cliente_LocalidadCli">
...
<?php if ($dbo_cliente->DescrProvincia->Visible) { // DescrProvincia ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->DescrProvincia) == "") { ?>
		<td><div id="elh_dbo_cliente_DescrProvincia" class="dbo_cliente_DescrProvincia"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->DescrProvincia->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->DescrProvincia) ?>',1);"><div id="elh_dbo_cliente_DescrProvincia" class="dbo_cliente_DescrProvincia">
...
<?php if ($dbo_cliente->CodigoPostalCli->Visible) { // CodigoPostalCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->CodigoPostalCli) == "") { ?>
		<td><div id="elh_dbo_cliente_CodigoPostalCli" class="dbo_cliente_CodigoPostalCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->CodigoPostalCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->CodigoPostalCli) ?>',1);"><div id="elh_dbo_cliente_CodigoPostalCli" class="dbo_cliente_CodigoPostalCli">
...
<?php if ($dbo_cliente->DescrPais->Visible) { // DescrPais ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->DescrPais) == "") { ?>
		<td><div id="elh_dbo_cliente_DescrPais" class="dbo_cliente_DescrPais"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->DescrPais->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->DescrPais) ?>',1);"><div id="elh_dbo_cliente_DescrPais" class="dbo_cliente_DescrPais">
...
<?php if ($dbo_cliente->Telefono->Visible) { // Telefono ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->Telefono) == "") { ?>
		<td><div id="elh_dbo_cliente_Telefono" class="dbo_cliente_Telefono"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->Telefono->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->Telefono) ?>',1);"><div id="elh_dbo_cliente_Telefono" class="dbo_cliente_Telefono">
...
<?php if ($dbo_cliente->FaxCli->Visible) { // FaxCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->FaxCli) == "") { ?>
		<td><div id="elh_dbo_cliente_FaxCli" class="dbo_cliente_FaxCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->FaxCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->FaxCli) ?>',1);"><div id="elh_dbo_cliente_FaxCli" class="dbo_cliente_FaxCli">
...
<?php if ($dbo_cliente->PaginaWebCli->Visible) { // PaginaWebCli ?>
	<?php if ($dbo_cliente->SortUrl($dbo_cliente->PaginaWebCli) == "") { ?>
		<td><div id="elh_dbo_cliente_PaginaWebCli" class="dbo_cliente_PaginaWebCli"><div class="ewTableHeaderCaption"><?php echo $dbo_cliente->PaginaWebCli->FldCaption() ?></div></div></td>
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $dbo_cliente->SortUrl($dbo_cliente->PaginaWebCli) ?>',1);"><div id="elh_dbo_cliente_PaginaWebCli" class="dbo_cliente_PaginaWebCli">
