...
function &DbHelper($dbid = "") {
	if ($dbid == "walger1" || $dbid === 1) // walger1
		$dbclass = "cwalger1_db";
	else // DB
		$dbclass = "cwalger_db";
...
function CurrentProjectID() {
	...
	return "{8B1B047A-723B-431D-9264-98AA1CD60F92}";
}
	...
	function ValidateUser(&$usr, &$pwd, $autologin, $encrypted = FALSE) {
		...
		if (!$validateUser) {
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$validateUser = $customValidateUser || ew_ComparePassword($rs->fields('contrasenia'), $pwd, $encrypted);
					if ($validateUser) {
						$_SESSION[EW_SESSION_STATUS] = "login";
