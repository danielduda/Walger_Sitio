...
$trama_mensajes2Dclientes = NULL;

//
// Table class for trama_mensajes-clientes
//
class ctrama_mensajes2Dclientes extends cTable {
	var $id;
	var $idCliente;
	var $fecha;
	var $mensaje;
	...
	function __construct() {
		...
		$this->TableVar = 'trama_mensajes2Dclientes';
		$this->TableName = 'trama_mensajes-clientes';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`trama_mensajes-clientes`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('trama_mensajes2Dclientes', 'trama_mensajes-clientes', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = FALSE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// idCliente
		$this->idCliente = new cField('trama_mensajes2Dclientes', 'trama_mensajes-clientes', 'x_idCliente', 'idCliente', '`idCliente`', '`idCliente`', 200, -1, FALSE, '`idCliente`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// fecha
		$this->fecha = new cField('trama_mensajes2Dclientes', 'trama_mensajes-clientes', 'x_fecha', 'fecha', '`fecha`', 'DATE_FORMAT(`fecha`, \'%Y/%m/%d\')', 135, 2, FALSE, '`fecha`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// mensaje
		$this->mensaje = new cField('trama_mensajes2Dclientes', 'trama_mensajes-clientes', 'x_mensaje', 'mensaje', '`mensaje`', '`mensaje`', 201, -1, FALSE, '`mensaje`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->mensaje->Sortable = TRUE; // Allow sort
		$this->fields['mensaje'] = &$this->mensaje;

		// leido
		$this->leido = new cField('trama_mensajes2Dclientes', 'trama_mensajes-clientes', 'x_leido', 'leido', '`leido`', '`leido`', 19, -1, FALSE, '`leido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
	...
	function GetMasterFilter() {
		...
		// Master filter
		$sMasterFilter = "";
		if ($this->getCurrentMasterTable() == "dbo_cliente") {
			if ($this->idCliente->getSessionValue() <> "")
				$sMasterFilter .= "`CodigoCli`=" . ew_QuotedValue($this->idCliente->getSessionValue(), EW_DATATYPE_STRING, "DB");
			else
				return "";
		}
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "dbo_cliente") {
			if ($this->idCliente->getSessionValue() <> "")
				$sDetailFilter .= "`idCliente`=" . ew_QuotedValue($this->idCliente->getSessionValue(), EW_DATATYPE_STRING, "DB");
			else
				return "";
		}
	...
	// Master filter
	function SqlMasterFilter_dbo_cliente() {
		return "`CodigoCli`='@CodigoCli@'";
	}

	// Detail filter
	function SqlDetailFilter_dbo_cliente() {
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`trama_mensajes-clientes`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "trama_mensajes2Dclienteslist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("trama_mensajes2Dclientesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("trama_mensajes2Dclientesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "trama_mensajes2Dclientesadd.php?" . $this->UrlParm($parm);
		else
			$url = "trama_mensajes2Dclientesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("trama_mensajes2Dclientesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("trama_mensajes2Dclientesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("trama_mensajes2Dclientesdelete.php", $this->UrlParm());
	}

	// Add master url
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "dbo_cliente" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
			$url .= (strpos($url, "?") !== FALSE ? "&" : "?") . EW_TABLE_SHOW_MASTER . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_CodigoCli=" . urlencode($this->idCliente->CurrentValue);
		}
		return $url;
	}
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->fecha->EditValue = ew_FormatDateTime($this->fecha->CurrentValue, 2);
