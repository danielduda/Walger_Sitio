...
function enviarMail($demail, $denombre, $paramail, $paranombre, $responderamail, $responderanombre, $concopia, $concopiaoculta, $asunto, $mensajehtml, $mensajeplano, $encabezado){
	...
	$body  = file_get_contents((isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://'.$GLOBALS["configuracion"]["host"]."/inc/render_emails.php", false, $cxContext);
...
function retornarPrimeraImagen($string){
	$imagenes= explode(',', $string);
	return $imagenes[0];
}
