...
$trama_portfolio = NULL;

//
// Table class for trama_portfolio
//
class ctrama_portfolio extends cTable {
	var $id;
	var $fkCategoriaPortfolio;
	var $denominacion;
	var $contenido;
	var $imagen;
	...
	function __construct() {
		...
		$this->TableVar = 'trama_portfolio';
		$this->TableName = 'trama_portfolio';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`trama_portfolio`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('trama_portfolio', 'trama_portfolio', 'x_id', 'id', '`id`', '`id`', 19, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// fkCategoriaPortfolio
		$this->fkCategoriaPortfolio = new cField('trama_portfolio', 'trama_portfolio', 'x_fkCategoriaPortfolio', 'fkCategoriaPortfolio', '`fkCategoriaPortfolio`', '`fkCategoriaPortfolio`', 19, -1, FALSE, '`fkCategoriaPortfolio`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->fkCategoriaPortfolio->Sortable = TRUE; // Allow sort
		$this->fkCategoriaPortfolio->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->fkCategoriaPortfolio->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->fkCategoriaPortfolio->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['fkCategoriaPortfolio'] = &$this->fkCategoriaPortfolio;

		// denominacion
		$this->denominacion = new cField('trama_portfolio', 'trama_portfolio', 'x_denominacion', 'denominacion', '`denominacion`', '`denominacion`', 200, -1, FALSE, '`denominacion`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->denominacion->Sortable = TRUE; // Allow sort
		$this->fields['denominacion'] = &$this->denominacion;

		// contenido
		$this->contenido = new cField('trama_portfolio', 'trama_portfolio', 'x_contenido', 'contenido', '`contenido`', '`contenido`', 201, -1, FALSE, '`contenido`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->contenido->Sortable = TRUE; // Allow sort
		$this->fields['contenido'] = &$this->contenido;

		// imagen
		$this->imagen = new cField('trama_portfolio', 'trama_portfolio', 'x_imagen', 'imagen', '`imagen`', '`imagen`', 200, -1, TRUE, '`imagen`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`trama_portfolio`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "trama_portfoliolist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("trama_portfolioview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("trama_portfolioview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "trama_portfolioadd.php?" . $this->UrlParm($parm);
		else
			$url = "trama_portfolioadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("trama_portfolioedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("trama_portfolioadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("trama_portfoliodelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->fkCategoriaPortfolio->Exportable) $Doc->ExportCaption($this->fkCategoriaPortfolio);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->fkCategoriaPortfolio->Exportable) $Doc->ExportField($this->fkCategoriaPortfolio);
