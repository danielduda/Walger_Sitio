...
if (@$_POST["a_edit"] <> "") {
	$dbo_cliente->CurrentAction = $_POST["a_edit"]; // Get action code
	LoadFormValues(); // Get form values
} else {
	$dbo_cliente->CurrentAction = "I"; // Default action is display
}

// Check if valid key
if ($dbo_cliente->CodigoCli->CurrentValue == "") Page_Terminate($dbo_cliente->getReturnUrl()); // Invalid key, exit
switch ($dbo_cliente->CurrentAction) {
	case "I": // Get a record to display
		...
		break;
	Case "U": // Update
		$dbo_cliente->SendEmail = TRUE; // Send email on update success
...
// Render the record
$dbo_cliente->RowType = EW_ROWTYPE_EDIT; // Render as edit
RenderRow();
?>
...
<p><span class="phpmaker">Editar : Clientes (ISIS)<br><br><a href="<?php echo $dbo_cliente->getReturnUrl() ?>"></a></span></p>
<?php
...
?>
<form name="fdbo_clienteedit" id="fdbo_clienteedit" action="dbo_clienteedit.php" method="post" onSubmit="return ew_ValidateForm(this);">
<p>
...
<div<?php echo $dbo_cliente->CodigoCli->ViewAttributes() ?>><?php echo $dbo_cliente->CodigoCli->EditValue ?></div>
<input type="hidden" name="x_CodigoCli" id="x_CodigoCli" value="<?php echo ew_HtmlEncode($dbo_cliente->CodigoCli->CurrentValue) ?>">
</span></td>
	</tr>
...
<!--
var f = document.fdbo_clienteedit;
...
function RenderRow() {
	...
	if ($dbo_cliente->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		$dbo_cliente->CodigoCli->EditValue = $dbo_cliente->CodigoCli->CurrentValue;
...
function EditRow() {
	...
	if ($rs->EOF) {
		...
		$bUpdateRow = $dbo_cliente->Row_Updating($rsold, $rsnew);
		if ($bUpdateRow) {
			$conn->raiseErrorFn = 'ew_ErrorFn';
			$EditRow = $conn->Execute($dbo_cliente->UpdateSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
	...
	if ($EditRow) {
		$dbo_cliente->Row_Updated($rsold, $rsnew);
	}
	$rs->Close();
	return $EditRow;
}
