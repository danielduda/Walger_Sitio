...
if (@$_POST["a_add"] <> "") {
  $walger_items_pedidos->CurrentAction = $_POST["a_add"]; // Get form action
  LoadFormValues(); // Load form values
} else { // Not post back
  if ($bCopy) {
    $walger_items_pedidos->CurrentAction = "C"; // Copy Record
  } else {
    $walger_items_pedidos->CurrentAction = "I"; // Display Blank Record
...
switch ($walger_items_pedidos->CurrentAction) {
  ...
  case "C": // Copy an existing record
   if (!LoadRow()) { // Load record based on key
      ...
      Page_Terminate($walger_items_pedidos->getReturnUrl()); // Clean up and return
    }
		break;
  case "A": // ' Add new record
		$walger_items_pedidos->SendEmail = TRUE; // Send email on add success
    if (AddRow()) { // Add successful
      $_SESSION[EW_SESSION_MESSAGE] = "Se ha guardado el nuevo registro"; // Set up success message
      Page_Terminate($walger_items_pedidos->KeyUrl($walger_items_pedidos->getReturnUrl())); // Clean up and return
    } else {
...
$walger_items_pedidos->RowType = EW_ROWTYPE_ADD;  // Render add type
RenderRow();
?>
...
function ew_ValidateForm(fobj) {
	...
	for (i=0; i<rowcnt; i++) {
		...
		if (elm && !ew_HasValue(elm)) {
			if (!ew_OnError(elm, "Ingrese el campo requerido - ID Pedido"))
				return false;
		}
		elm = fobj.elements["x" + infix + "_idPedido"];
		if (elm && !ew_CheckInteger(elm.value)) {
			if (!ew_OnError(elm, "Entero Incorrecto - ID Pedido"))
				return false; 
		}
		...
		if (elm && !ew_HasValue(elm)) {
			if (!ew_OnError(elm, "Ingrese el campo requerido - Precio"))
				return false;
		}
		elm = fobj.elements["x" + infix + "_precio"];
		if (elm && !ew_CheckNumber(elm.value)) {
			if (!ew_OnError(elm, "Valor decimal incorrecto - Precio"))
				return false; 
		}
		elm = fobj.elements["x" + infix + "_cantidad"];
		if (elm && !ew_HasValue(elm)) {
			if (!ew_OnError(elm, "Ingrese el campo requerido - Cantidad"))
				return false;
		}
		elm = fobj.elements["x" + infix + "_cantidad"];
		if (elm && !ew_CheckInteger(elm.value)) {
			if (!ew_OnError(elm, "Entero Incorrecto - Cantidad"))
				return false; 
		}
...
<p><span class="phpmaker">Agregar a : Items Pedidos<br><br><a href="<?php echo $walger_items_pedidos->getReturnUrl() ?>"></a></span></p>
<?php
...
?>
<form name="fwalger_items_pedidosadd" id="fwalger_items_pedidosadd" action="walger_items_pedidosadd.php" method="post" onSubmit="return ew_ValidateForm(this);">
<p>
...
<?php if ($walger_items_pedidos->idPedido->getSessionValue() <> "") { ?>
<div<?php echo $walger_items_pedidos->idPedido->ViewAttributes() ?>><?php echo $walger_items_pedidos->idPedido->ViewValue ?></div>
<input type="hidden" id="x_idPedido" name="x_idPedido" value="<?php echo ew_HtmlEncode($walger_items_pedidos->idPedido->CurrentValue) ?>">
<?php } else { ?>
<input type="text" name="x_idPedido" id="x_idPedido" title="" size="30" value="<?php echo $walger_items_pedidos->idPedido->EditValue ?>"<?php echo $walger_items_pedidos->idPedido->EditAttributes() ?>>
<?php } ?>
</span></td>
  </tr>
...
<select id="x_CodInternoArti" name="x_CodInternoArti" onChange="ew_UpdateOpt(this.form.x_CodInternoArti, ar_x_CodInternoArti, this);"<?php echo $walger_items_pedidos->CodInternoArti->EditAttributes() ?>>
<!--option value="">Seleccione</option-->
<?php
if (is_array($walger_items_pedidos->CodInternoArti->EditValue)) {
	$arwrk = $walger_items_pedidos->CodInternoArti->EditValue;
	$rowswrk = count($arwrk);
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($walger_items_pedidos->CodInternoArti->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected" : "";	
?>
...
<input type="text" name="x_precio" id="x_precio" title="" size="30" value="<?php echo $walger_items_pedidos->precio->EditValue ?>"<?php echo $walger_items_pedidos->precio->EditAttributes() ?>>
</span></td>
  </tr>
  <tr class="ewTableAltRow">
...
<input type="text" name="x_cantidad" id="x_cantidad" title="" size="30" value="<?php echo $walger_items_pedidos->cantidad->EditValue ?>"<?php echo $walger_items_pedidos->cantidad->EditAttributes() ?>>
</span></td>
  </tr>
  <tr class="ewTableRow">
...
<!--
var f = document.fwalger_items_pedidosadd;
...
function RenderRow() {
	...
	if ($walger_items_pedidos->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		if ($walger_items_pedidos->idPedido->getSessionValue() <> "") {
			$walger_items_pedidos->idPedido->CurrentValue = $walger_items_pedidos->idPedido->getSessionValue();
		...
		$walger_items_pedidos->idPedido->EditValue = ew_HtmlEncode($walger_items_pedidos->idPedido->CurrentValue);
		}
		...
		$walger_items_pedidos->CodInternoArti->EditValue = $arwrk;

		// precio
		$walger_items_pedidos->precio->EditCustomAttributes = "";
		$walger_items_pedidos->precio->EditValue = ew_HtmlEncode($walger_items_pedidos->precio->CurrentValue);

		// cantidad
		$walger_items_pedidos->cantidad->EditCustomAttributes = "";
		$walger_items_pedidos->cantidad->EditValue = ew_HtmlEncode($walger_items_pedidos->cantidad->CurrentValue);
...
function AddRow() {
	...
	$bCheckKey = TRUE;
	$sFilter = $walger_items_pedidos->SqlKeyFilter();
	if (trim(strval($walger_items_pedidos->idItemPedido->CurrentValue)) == "") {
		$bCheckKey = FALSE;
	} else {
		$sFilter = str_replace("@idItemPedido@", ew_AdjustSql($walger_items_pedidos->idItemPedido->CurrentValue), $sFilter); // Replace key value
	}
	...
	if ($bCheckKey) {
		$rsChk = $walger_items_pedidos->LoadRs($sFilter);
		if ($rsChk && !$rsChk->EOF) {
	...
	// Field idPedido
	$walger_items_pedidos->idPedido->SetDbValueDef($walger_items_pedidos->idPedido->CurrentValue, 0);
	$rsnew['idPedido'] =& $walger_items_pedidos->idPedido->DbValue;

	// Field CodInternoArti
	$walger_items_pedidos->CodInternoArti->SetDbValueDef($walger_items_pedidos->CodInternoArti->CurrentValue, "");
	$rsnew['CodInternoArti'] =& $walger_items_pedidos->CodInternoArti->DbValue;

	// Field precio
	$walger_items_pedidos->precio->SetDbValueDef($walger_items_pedidos->precio->CurrentValue, 0);
	$rsnew['precio'] =& $walger_items_pedidos->precio->DbValue;

	// Field cantidad
	$walger_items_pedidos->cantidad->SetDbValueDef($walger_items_pedidos->cantidad->CurrentValue, 0);
	$rsnew['cantidad'] =& $walger_items_pedidos->cantidad->DbValue;
	...
	$bInsertRow = $walger_items_pedidos->Row_Inserting($rsnew);
	if ($bInsertRow) {
		$conn->raiseErrorFn = 'ew_ErrorFn';
		$AddRow = $conn->Execute($walger_items_pedidos->InsertSQL($rsnew));
		$conn->raiseErrorFn = '';
	} else {
	...
	if ($AddRow) {
		$walger_items_pedidos->idItemPedido->setDbValue($conn->Insert_ID());
		$rsnew['idItemPedido'] =& $walger_items_pedidos->idItemPedido->DbValue;

		// Call Row Inserted event
		$walger_items_pedidos->Row_Inserted($rsnew);
	}
	return $AddRow;
}
?>
