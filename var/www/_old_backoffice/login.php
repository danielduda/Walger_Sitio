...
if ($bValidate) {
	...
	if ($bValidate) {
		...
		if (!$bValidPwd) $_SESSION[EW_SESSION_MESSAGE] = "Usuario o contraseña inválida"; // Invalid User ID/password
	} else {
		if (@$_SESSION[EW_SESSION_MESSAGE] == "") $_SESSION[EW_SESSION_MESSAGE] = "Ingresar cancelado"; // Login cancelled
	}
	if ($bValidPwd) {
...
function ew_ValidateForm(fobj) {
	if (!ew_HasValue(fobj.username)) {
		if  (!ew_OnError(fobj.username, "Escriba el id de usuario"))
			return false;
	}
	if (!ew_HasValue(fobj.password)) {
		if (!ew_OnError(fobj.password, "Escriba su contraseña"))
			return false;
	}
	return true;
}
...
<p><span class="phpmaker">Página de ingreso</span></p>
<?php
...
<table border="0" cellspacing="0" cellpadding="4">
	...
	<tr>
		...
		<input type="radio" name="rememberme" id="rememberme" value="a" checked>Autoconectarse mientras no haga clic en salir<br><input type="radio" name="rememberme" id="rememberme" value="u">Guardar mi usuario<br><input type="radio" name="rememberme" id="rememberme" value="n">Siempre preguntar mi usuario y contraseña
		<?php } elseif (@$_COOKIE[EW_PROJECT_NAME]['AutoLogin'] == "rememberusername") { ?>
		<input type="radio" name="rememberme" id="rememberme" value="a">Autoconectarse mientras no haga clic en salir<br><input type="radio" name="rememberme" id="rememberme" value="u" checked>Guardar mi usuario<br><input type="radio" name="rememberme" id="rememberme" value="n">Siempre preguntar mi usuario y contraseña
		<?php } else { ?>
		<input type="radio" name="rememberme" id="rememberme" value="a">Autoconectarse mientras no haga clic en salir<br><input type="radio" name="rememberme" id="rememberme" value="u">Guardar mi usuario<br><input type="radio" name="rememberme" id="rememberme" value="n" checked>Siempre preguntar mi usuario y contraseña
		<?php } ?>
		</span></td>
	</tr>
	</tr>
